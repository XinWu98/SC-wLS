...
parser.add_argument(
    "--loss_essential_init_epo", type=int, default=0, help=""
...
parser.add_argument('--useRANSAC', action='store_true',
                    help='will use RANSAC for pose estimation')
# Dsacstar ransac
...
def main():
    ...
    args.save_path = 'checkpoints/{}/{}/{}'.format(args.model, args.dataset, scene) / save_path
...
def train(args, epoch, train_loader, SC_Net, optimizer, epoch_size, logger, train_writer, max_grad_norm=0.0):
    ...
    for i, frames in enumerate(train_loader):
        ...
        loss_1, loss_2, loss_3, pred_X_3d, uncertainty, e_hat_post, prediction_grid_pad,d,r = SC_Net(X_world, image, Ts,
                                                                                                Ks, epoch, test=False, ds=False)
        ...
        if log_losses:
            train_writer.add_scalar('e2e_loss', loss_1.item(), n_iter)
            train_writer.add_scalar('pose_loss', loss_2.item(), n_iter)
            train_writer.add_scalar('cla_loss', loss_3.item(), n_iter)
            train_writer.add_scalar('d_term', d, n_iter)
            train_writer.add_scalar('r_term', r, n_iter)
...
def validate_with_gt(args, val_loader, SC_Net, epoch, logger, output_writers=[]):
    ...
    for i, frames in enumerate(val_loader):
        ...
        if log_val_loss:
            loss_1, loss_2, loss_3, pred_X_3d, uncertainty, e_hat_post, prediction_grid_pad,d,r = SC_Net(None, image, Ts,
                                                                                                    Ks, epoch, test=False, ds=False)
            ...
            log_outputs = 1  
            if log_outputs: 
                output_writers.add_scalar('e2e_loss_val', loss_1.item(), val_n_iter)
                output_writers.add_scalar('pose_loss_val', loss_2.item(), val_n_iter)
                output_writers.add_scalar('cla_loss_val', loss_3.item(), val_n_iter)
                output_writers.add_scalar('d_term_val', d, val_n_iter)
                output_writers.add_scalar('r_term_val', r, val_n_iter)
        else:
...
if __name__ == '__main__':
    ...
    # Set False when training, since the resolutions of image are diff
